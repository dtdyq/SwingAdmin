 ...
 * @author Karl Tauber
 */
class FlatCompletionProvider extends CompletionProviderBase {
    ...
    private CompletionProvider getProviderFor(JTextComponent comp) {
        ...
        try {
            ...
            if (lineBeforeCaret.trim().startsWith("#")) return null;

            // key
            if (lineBeforeCaret.indexOf('=') < 0) return getKeyProvider();
    ...
    private CompletionProvider getKeyProvider() {
        if (keyProvider == null) keyProvider = KeyCompletionProvider.getInstance();
        return keyProvider;
    }

    private CompletionProvider getReferenceProvider() {
        if (referenceProvider == null) referenceProvider = new ReferenceCompletionProvider();
        return referenceProvider;
    }

    private CompletionProvider getValueProvider() {
        if (valueProvider == null) valueProvider = new ValueCompletionProvider(getReferenceProvider());
     ...
     */
    private static final class KeyCompletionProvider extends BaseCompletionProvider {
        ...
        static KeyCompletionProvider getInstance() {
            if (instance == null) instance = new KeyCompletionProvider();
            return instance;
        }
                    ...
                    if (in != null) {
                        try (BufferedReader reader = new BufferedReader(new InputStreamReader(in, "UTF-8"))) {
                            ...
                            while ((key = reader.readLine()) != null) {
                                if (!isIgnored(key)) knownKeys.add(key);
                            }
                        }
                    }
                }
        ...
        private void setCompletions(Set<String> keys) {
            ...
            // add all keys
            for (String key : keys) {
                if (!keyParts.contains(key)) completions.add(new BasicCompletion(this, key.concat(" = ")));
            }
        ...
        private void updateCompletions(JTextComponent comp) {
            ...
            if (Objects.equals(keys, lastKeys)) return;
            lastKeys = keys;
            ...
            try {
                ...
                if (!properties.isEmpty()) keyAtCurrentLine = (String) properties.keys().nextElement();
            ...
            for (String key : keys) {
                ...
                if (key.equals(keyAtCurrentLine)) continue;
    ...
    private static class BaseCompletionProvider extends DefaultCompletionProvider {
        ...
        public boolean isAutoActivateOkay(JTextComponent comp) {
            ...
            try {
                ...
                return (autoActivateAfterLetters && Character.isLetter(ch)) || (autoActivateChars != null && autoActivateChars.indexOf(ch) >= 0);
     ...
     */
    private static class ReferenceCompletionProvider extends BaseCompletionProvider {
        private Set<String> lastKeys;
            ...
            for (String key : keys) {
                ...
                if (key.startsWith("*.")) {
                    ...
                    for (String k : FlatThemePropertiesSupport.getKeysForWildcard(key)) {
                        if (!keys.contains(k)) completions.add(new BasicCompletion(this, "$".concat(k)));
                    }
                    continue;
                }
                ...
                if (key.startsWith("@")) completion.setRelevance(1);
     ...
     */
    private static class ValueCompletionProvider extends BaseCompletionProvider implements ParameterChoicesProvider {
        ValueCompletionProvider(CompletionProvider parent) {
            ...
            addFunction("if", "condition", "evaluates to true if: is not \"null\" and is not \"false\" and is not an integer with zero value", "trueValue", "used if condition is true", "falseValue", "used if condition is false");
            addFunction("lazy", "uiKey", "UI key (without leading '$')");

            addFunction("systemColor", "name", "system color name", "defaultValue", "default color value used if system color is not available");

            addFunction("rgb", "red", "0-255 or 0-100%", "green", "0-255 or 0-100%", "blue", "0-255 or 0-100%");
            addFunction("rgba", "red", "0-255 or 0-100%", "green", "0-255 or 0-100%", "blue", "0-255 or 0-100%", "alpha", "0-255 or 0-100%");

            addFunction("hsl", "hue", "0-360 representing degrees", "saturation", "0-100%", "lightness", "0-100%");
            addFunction("hsla", "hue", "0-360 representing degrees", "saturation", "0-100%", "lightness", "0-100%", "alpha", "0-100%");

            String colorParamDesc = "a color (e.g. #f00), a reference (e.g. $Other.key) or a color function";
            String[] hslIncreaseDecreaseParams = {"color", colorParamDesc, "amount", "0-100%", "options", "(optional) [relative] [autoInverse] [noAutoInverse] [lazy] [derived]"};
            ...
            addFunction("fade", "color", colorParamDesc, "amount", "0-100%", "options", "(optional) [derived]");
            addFunction("spin", "color", colorParamDesc, "angle", "number of degrees to rotate (0-360)", "options", "(optional) [derived]");

            addFunction("changeHue", "color", colorParamDesc, "angle", "number of degrees (0-360)", "options", "(optional) [derived]");
            String[] hslChangeParams = {"color", colorParamDesc, "value", "0-100%", "options", "(optional) [derived]"};
            ...
            addFunction("mix", "color1", colorParamDesc, "color2", colorParamDesc, "weight", weightParamDesc);
            addFunction("tint", "color", colorParamDesc, "weight", weightParamDesc);
            addFunction("shade", "color", colorParamDesc, "weight", weightParamDesc);

            addFunction("contrast", "color", colorParamDesc, "dark", colorParamDesc, "light", colorParamDesc, "threshold", "(optional) 0-100%, default is 43%");

            addFunction("over", "foreground", colorParamDesc, "background", colorParamDesc);
        }
