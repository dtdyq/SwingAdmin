	...
	extends JPanel
{
	...
	public IJThemesPanel() {
		...
		saveButton.setIcon( new FlatSVGIcon( "icons/download.svg" ) );
		sourceCodeButton.setIcon( new FlatSVGIcon( "icons/github.svg" ) );
	...
	private void setTheme( IJThemeInfo themeInfo ) {
		...
		if( themeInfo.lafClassName != null ) {
			...
			try {
				...
				AdminPrefs.getState().put( AdminPrefs.KEY_LAF_THEME, AdminPrefs.FILE_PREFIX + themeInfo.themeFile );
			} catch( Exception ex ) {
				LoggingFacade.INSTANCE.logSevere( null, ex );
		    ...
		    AdminPrefs.getState().put( AdminPrefs.KEY_LAF_THEME, AdminPrefs.RESOURCE_PREFIX + themeInfo.resourceName );
		}
	...
	private void selectedCurrentLookAndFeel() {
		...
		String theme = UIManager.getLookAndFeelDefaults().getString( AdminPrefs.THEME_UI_KEY );
		...
		if( theme != null && theme.startsWith( AdminPrefs.RESOURCE_PREFIX ) ) {
			String resourceName = theme.substring( AdminPrefs.RESOURCE_PREFIX.length() );
			test = ti -> Objects.equals( ti.resourceName, resourceName );
		} else if( theme != null && theme.startsWith( AdminPrefs.FILE_PREFIX ) ) {
			File themeFile = new File( theme.substring( AdminPrefs.FILE_PREFIX.length() ) );
